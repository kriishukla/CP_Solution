...
vi cost,dd,brr,hh,w,pp,crr;
 vvi g;
 vector<vector<char>> arr;

void rec(lli lvl,lli r1,lli c1){
   if(lvl==0)return;
   lli x=bin_pow<lli>(3,lvl-1,M);
   for(int i=r1+x;i<=r1+2*x-1;i++){
      for(int j=c1+x;j<=c1+2*x-1;j++){
          arr[i][j]='.';
      }
   }
   rec(lvl-1,r1,c1);
   ...
   rec(lvl-1,r1,c1+x);
   rec(lvl-1,r1,c1+2*x);
   rec(lvl-1,r1+2*x,c1+x);
   rec(lvl-1,r1+2*x,c1+2*x);
   rec(lvl-1,r1+x,c1+2*x);
}


void solve1(){
  cin>>n;
  lli x=bin_pow<lli>(3,n,M);
  arr.assign(x,vector<char>(x,'#'));
 rec(n,0,0);
 for(int i=0;i<x;i++){
   for(int j=0;j<x;j++){
      cout<<arr[i][j];
