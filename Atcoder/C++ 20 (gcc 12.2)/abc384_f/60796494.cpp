...
lli gg(lli p,lli q){
  ...
  for(int i=0;i<4;i++){
    ...
    lli c=q+dy[i];
    if(is_valid(r,c)){
     st.insert(MP(arr[r][c],MP(r,c)));
    }
  }
  vis[p][q]=1;
  while(!st.empty()){
    auto [wt,r_c]=*st.begin();
    auto [r,c]=r_c;
    // deb(r_c);
    st.erase(MP(wt,r_c));
    lli num=x;
    if(x>1e18/arr[r][c])num=1e18;
    else num=x*arr[r][c];
   ...
   for(int i=0;i<4;i++){
    lli r1=r+dx[i];
    lli c1=c+dy[i];
    if(is_valid(r1,c1) && vis[r1][c1]==0){
     st.insert(MP(arr[r1][c1],MP(r1,c1)));
    }
  }
    }
    else{
