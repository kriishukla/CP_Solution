...
lli rec(lli lvl,lli w){
  ...
  if(!arr[lvl].empty()){
    ...
    lli dp1[w/lvl+1]={0};
    multiset<lli> mt;
    for(int i=0;i<arr[lvl].size();i++){
       mt.insert(arr[lvl][i]-2*1+1);
    }
    for(int i=1;i<=w/lvl;i++){
      lli num=*(mt.rbegin());
       dp1[i]=dp1[i-1]+num;
       mt.erase(mt.find(num));
      mt.insert(num-2);
      ans=max(ans,dp1[i]+rec(lvl+1,w-i*lvl));
    }
  //  for(int i=1;i<=w/lvl;i++){
  //  cout<<dp[i]<<" ";
  //  }
