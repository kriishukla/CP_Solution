...
lli rec(lli lvl,lli max_,lli take){
  if(take>k)return 0;
  if(lvl==-1){
    if(take==k)return 1;
    return 0;
  }
  if(dp[lvl][max_][take]!=-1){
    return dp[lvl][max_][take];
  }
  lli ans=0;
  if(arr[lvl].S<=max_){
    ans+=rec(lvl-1,max_,take+1);
  }
  else{
    ans+=rec(lvl-1,max(max_,arr[lvl].S),take+1);
    ans%=M;
    ans+=rec(lvl-1,max_,take);
  }
  ans%=M;
  return dp[lvl][max_][take]=ans;
}
