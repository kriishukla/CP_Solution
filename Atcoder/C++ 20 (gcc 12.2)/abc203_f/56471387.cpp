...
lli n,m,x,k,tt,q,p,y,z,h,c,w,l,r,d;
lli lcm(lli a,lli b){
...
lli rec(lli lvl,lli move){
  if(move<0)return 1e17;
  if(lvl==-1){
    return 0;
  }
  if(dp[lvl][move]!=-1){
  ...
  lli ans=1+rec(lvl-1,move);
  lli h=arr[lvl];
  h/=2;
  if(move){
   lli pos=ub(all(arr),h)-arr.begin();
   pos--;
   ans=min(ans,rec(pos,move-1));
  }
  return dp[lvl][move]=ans;
}
...
void solve(){
 ...
 sort(all(arr));
 // dp[i][j]-> from 0 to i if Takahashi does j operation then what is the min k
 lli min_=1e17;
 for(int i=0;i<40;i++){
  // deb2(rec(n-1,i),i);
  if(rec(n-1,i)<=k){
    cout<<i<<" "<<rec(n-1,i)<<endl;
    return;
  }
 }
}
