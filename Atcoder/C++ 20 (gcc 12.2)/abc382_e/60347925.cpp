...
ld rec(lli lvl,lli nn){
  ...
  ld ans=arr[lvl]*rec(lvl+1,nn-1);
  ans=ans+(1-arr[lvl])*rec(lvl+1,nn);
  test[lvl][nn]=tt;
  return dp[lvl][nn]=ans;
}
...
ld rec1(lli x){
  ...
  ld ans=0;
  for(int i=1;i<=n;i++){
    ans=ans+rec(0,i)*(1+rec1(x-i));
  }
  ans=ans+rec(0,0);
  ans=(1.0*ans)/(1-rec(0,0));
...
lli solve(lli n1,lli m1){
  ...
  cout<<fixed<<setprecision(10)<<rec1(x)<<endl;
