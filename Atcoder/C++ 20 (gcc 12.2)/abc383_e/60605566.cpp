...
lli rec(lli lvl,lli lft,lli taken){
  if(lft<0)return -1e17;
  if(lvl==n){
    return 0;
  }
  if(dp[lvl][lft][taken]!=-1){
    return dp[lvl][lft][taken];
  }
  auto [col,p_u]=v[lvl];
  auto [p,u]=p_u;
  lli ans=0;
  if(st[lvl]==1){
    ans=max({ans,u+k+rec(lvl+1,lft-p,0),rec(lvl+1,lft,1)});
  }
  else{
    if(taken){
      ans=max({ans,u+k+rec(lvl+1,lft-p,0),rec(lvl+1,lft,1)});
      ...
      ans=max({ans,u+rec(lvl+1,lft-p,0),rec(lvl+1,lft,0)});
    }
  }
  return dp[lvl][lft][taken]=ans;
}
