 ...
 for(int len=2;len<=n;len++){
   for(int curr=1;curr<=len;curr++){
    if(str[len-2]=='<'){
      ...
      dp[len][curr]=pref[len-1][curr-1]%M;
      pref[len][curr]+=(dp[len][curr]+pref[len][curr-1]);
      pref[len][curr]%=M;
    //  deb2(len,curr);
    //  deb2(dp[len][curr],pref[len-1][curr-1]);
    }
    else{
      dp[len][curr]=pref[len-1][len-1]-pref[len-1][curr-1];
      dp[len][curr]%=M;
      dp[len][curr]=(dp[len][curr]+M)%M;
       pref[len][curr]+=(dp[len][curr]+pref[len][curr-1]);
      pref[len][curr]%=M;
    }
   }
